        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <span className="text-[10px] text-slate-400">
              Deterministic Market Scanner &amp; AI Analysis
            </span>
          </div>
        </div>
                      ...
                      {msg.data.scanSummary.items.map((item: ScanResultItem) => (
                        ...
                        >
                          ...
                          <td className="py-2 px-2.5 text-right text-slate-300">
                            {formatINR(item.sma20)}
                          </td>
                          <td className="py-2 px-2.5 text-right text-slate-300">
                            {formatINR(item.sma50)}
                          </td>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      ...
                      <span className="text-sm font-mono font-bold text-cyan-400">
                        {msg.data.analysis.scoreBreakdown.totalScore}/10
                      </span>
                    </div>
                  </div>
                  ...
                  {onSelectInstrument && (
                    <Button
                      ...
                      onClick={() =>
                        onSelectInstrument(
                          msg.data!.analysis!.symbol,
                          msg.data!.analysis!.exchange as "NSE" | "BSE"
                        )
                      }
                      className="w-full text-xs text-cyan-400 hover:text-cyan-300 py-1.5 mt-2 border-slate-700"
                    >
              ...
              {msg.data?.type === "PORTFOLIO_REPORT" && msg.data.portfolioReport && (
                <div className="mt-3 p-3.5 rounded-xl border border-slate-800 bg-[#090d16] space-y-3">
                  ...
                  {msg.data.portfolioReport.warnings.map((w: any, idx: number) => (
                    <div
                      key={idx}
                      className="p-2.5 rounded-lg bg-amber-950/30 border border-amber-800/40 text-amber-300 text-xs flex items-start gap-2"
                    >
                      <AlertTriangle className="w-4 h-4 shrink-0 text-amber-400 mt-0.5" />
                      <div>
                        <span className="font-bold">{w.symbol}: </span>
      ...
      <div className="p-3 bg-[#0a0f18] border-t border-slate-800/80">
        ...
        <div className="flex items-center justify-between text-[10px] text-slate-500 mt-2 px-1">
          <span className="flex items-center gap-1">
            ...
            Deterministic math &bull; Zero live execution in Phase 1
          </span>
          <span className="font-mono">RESEARCH MODE</span>
        </div>
      </div>
